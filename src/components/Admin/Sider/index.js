...
import {  QuestionCircleOutlined, HighlightOutlined, MailOutlined } from '@ant-design/icons';
...
export default function AdminSider({ selectedRouters = [], redirectTo = () => {}, }) {
  ...
  const items = filterListByPermission([
    ...
    getItem('System settings', ROUTERS.ADMIN_SYSTEM_SETTINGS_MANAGEMENT, <Icon src={systemSettingIcon} activeSrc={systemSettingActiveIcon} active={checkRouterMatchFn(ROUTERS.ADMIN_SYSTEM_SETTINGS_MANAGEMENT)} width={22} height={22}/>, filterListByPermission([
      ...
      getItem('Blog categories', ROUTERS.ADMIN_SYSTEM_BLOG_CATEGORIES_MANAGEMENT, <Icon src={productCategoryIcon} activeSrc={productCategoryActiveIcon} active={checkRouterMatchFn(ROUTERS.ADMIN_SYSTEM_BLOG_CATEGORIES_MANAGEMENT)} />, undefined, undefined, authentication.getPermission(PERMISSION_VALUES.ADMIN_VIEW_BLOGS)),
      getItem('Blogs', ROUTERS.ADMIN_SYSTEM_BLOGS_MANAGEMENT, <HighlightOutlined style={{ marginRight: 14, marginLeft: 3, fontSize: 18, color: checkRouterMatchFn(ROUTERS.ADMIN_SYSTEM_BLOGS_MANAGEMENT) ? '#fff' : '#626F86' }} />, undefined, undefined, authentication.getPermission(PERMISSION_VALUES.ADMIN_VIEW_BLOGS)),
      getItem('Subscribers', ROUTERS.ADMIN_SYSTEM_EMAILS_MANAGEMENT, <MailOutlined style={{ marginRight: 14, marginLeft: 3, fontSize: 18, color: checkRouterMatchFn(ROUTERS.ADMIN_SYSTEM_EMAILS_MANAGEMENT) ? '#fff' : '#626F86' }} />, undefined, undefined, authentication.getPermission(PERMISSION_VALUES.ADMIN_VIEW_EMAILS)),
